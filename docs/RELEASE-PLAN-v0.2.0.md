...
| --- | --- | --- | --- |
| 1. Raw envelope | Decode the verified 116-byte stored representation into raw AS5643 fields and a borrowed 92-byte application region. | Golden fixture values, exact-length errors, explicit profile ID, preserved bytes, and L3 trace markers. | In progress |
| 2. Integrity | Calculate VPC from explicit reconstructed header inputs and return valid, invalid, absent, or unchecked outcomes. | Known-good and mutated fixtures with stored/calculated values retained. | Planned |
